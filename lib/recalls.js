...
export class RecallChecker {
  ...
  /**
   * Update the recall cache from CPSC
   */
  async updateRecallCache() {
    try {
      // Try CPSC SaferProducts API first
      const recalls = await this.fetchFromSaferProducts();

      if (recalls && recalls.length > 0) {
        await this.storage.saveRecallCache(recalls);
        console.log(`Updated recall cache with ${recalls.length} recalls`);
        return recalls;
      }
    } catch (error) {
      console.error('Error updating recall cache:', error);
    }
